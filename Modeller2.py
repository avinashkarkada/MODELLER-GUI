...
def run():
    ...
    env = environ()
    aln = alignment(env)
    mdl = model(env, file= onlyname2, model_segment=('FIRST:' + chain1, 'LAST:' + chain2))
...
E = Label(root, text = "Enter the chain(s) to be modelled:")
E.grid(row=5, column=1, sticky=W)


f = Entry(root, width = "10", textvariable= StringVar)
f.grid(row=6, column=2, sticky=W)
F = Label(root, text = "First:")
F.grid(row=6, column=1, sticky="E")


g = Entry(root, width = "10", textvariable= StringVar)
g.grid(row=7, column=2, sticky=W)
G = Label(root, text = "Last:")
G.grid(row=7, column=1, sticky="E")
